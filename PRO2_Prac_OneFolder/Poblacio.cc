...
void Poblacio::reproduir(string mare, string pare, string ID){
  ...
  if ((mare_it != poblacio.end()) and
      (pare_it != poblacio.end()) and
      (poblacio.find(ID) == poblacio.end())
     )
  {
...
void Poblacio::completar_arbre(queue<string>& arbre) {
  queue<string> otp;
  ...
  arbre.pop();
  cout << " ";
  if (arrel != poblacio.end() and input_and_completar_arbre(arrel, otp, arbre)) {
    while (not otp.empty()) {
          cout << ' ' << otp.front();
          otp.pop();
    }
