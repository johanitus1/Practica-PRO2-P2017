...
public:
   ...
   */
  void completar_arbre(queue<string>& arbre) ;
...
private:
  ...
  bool input_and_completar_arbre(map_iterator arrel, queue<string>& otp, queue<string>& arbre) {
    string pares;
    map_iterator p;
    bool valid = true;
    ...
    if (pares == "$") {
      ...
      if (poblacio.find(pares) != p) {
        valid = false;
          ...
          otp.push(pares);
          valid = input_and_completar_arbre(p , otp, arbre);
        }
      }
