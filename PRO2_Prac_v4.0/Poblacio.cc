	...
	if
	...
	else
		{
			if (posible_reproduccio(mare_it, pare_it))
			{
				...
				cin >> nm >> np >> t;
				sexe = (np == 1) ? 'Y' : 'X';
				c_s = creuaCromosomesSexuals(mare.consultarCromosomaSexual(nm), pare.consultarCromosomaSexual(np), t, l0);

				for (int i = 1; i <= N; ++i)
      	{
        	cin >> nm >> np >> t;
        	c_n[i-1] = creuaCromosomesNormals(mare.consultarCromosomaNormal(i, nm), pare.consultarCromosomaNormal(i, np), t);
      	}
